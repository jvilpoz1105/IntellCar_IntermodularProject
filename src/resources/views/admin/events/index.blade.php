...
@section('heading', 'Gestión de Eventos')
    ...
    <table class="w-full text-sm text-left">
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                <td class="px-6 py-3 text-gray-500 dark:text-gray-400">{{ $event->event_id }}</td>
                <td class="px-6 py-3 font-medium text-gray-900 dark:text-white max-w-[200px] truncate">
                    {{ $event->title }}
                </td>
                <td class="px-6 py-3 text-gray-600 dark:text-gray-300">
                    {{ $event->creator?->user_name ?? '—' }}
                </td>
                <td class="px-6 py-3 text-gray-600 dark:text-gray-300">
                    {{ $event->city ?? '—' }}
                </td>
                ...
                <td class="px-6 py-3">
                    @if ($event->onDeleteRequest)
                        <span class="text-xs text-orange-600 dark:text-orange-400 font-medium">
                            {{ $event->onDeleteRequest->format('d/m/Y') }}
                        </span>
                    @else
                ...
                <td class="px-6 py-3">
                    <div class="flex items-center gap-2">
                        ...
                        <form method="POST" action="{{ route('admin.events.toggle-visible', $event->event_id) }}">
                            @csrf @method('PATCH')
                            <button type="submit"
                                    class="text-xs font-medium {{ $event->visible ? 'text-orange-500 hover:text-orange-700' : 'text-green-600 hover:text-green-800' }}">
            ...
            @empty
            <tr>
                <td colspan="8" class="px-6 py-10 text-center text-slate-500">No se encontraron eventos.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
